...
point.prototype = {
	...
	// returns a point in the same direction, with length 1
	normed : function() {
		var len = Math.sqrt(Math.pow(this.x,2) + Math.pow(this.y,2));
		return new point( p.x/len, p.y/len );
	},
	
	// returns a point in the same direction, with a new magnitude
	withMag : function(mag) {
		var len = Math.sqrt(Math.pow(this.x,2) + Math.pow(this.y,2));
		return new point(this.x*(mag/len), this.y*(mag/len));
	},
	...
	// returns your distance from the nearest wall, given your position (this.x, this.y)
	// and your heading, as well as the bounding width and height
	d_from_wall : function(heading, width, height) {
		var hits_x_axis = (heading >= 0 && heading < Math.PI);
		var hits_y_axis = (heading>=Math.PI/2 && heading < Math.PI*3/2);
		
		
		// maxes with 1 prevent divide by 0 error
		if (hits_x_axis) { var x_near = this.x }
		else { var x_near = width - this.x };
		if (hits_y_axis) { var y_near = this.y }
		else { var y_near = height - this.y };
		
		x_axis_dist = Math.max(Math.abs(Math.cos(heading)/x_near));
		y_axis_dist = Math.max(Math.abs(Math.cos(heading)/y_near));
		
		return Math.min(x_axis_dist, y_axis_dist)
	}
}
